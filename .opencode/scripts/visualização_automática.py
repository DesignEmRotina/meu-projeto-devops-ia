```python
#!/usr/bin/env python3
"""
Auto Preview - Antigravity Kit
=============================
Gerencia (iniciar/parar/status) o servidor local de desenvolvimento para pré-visualização da aplicação.

Uso:
    python .agent/scripts/visualização_automática.py start [porta]
    python .agent/scripts/visualização_automática.py stop
    python .agent/scripts/visualização_automática.py status
"""

import os
import sys
import time
import json
import signal
import argparse
import subprocess
from pathlib import Path

AGENT_DIR = Path(".agent")
PID_FILE = AGENT_DIR / "preview.pid"
LOG_FILE = AGENT_DIR / "preview.log"

def get_project_root():
    return Path(".").resolve()

def is_running(pid):
    try:
        os.kill(pid, 0)
        return True
    except OSError:
        return False

def get_start_command(root):
    pkg_file = root / "package.json"
    if not pkg_file.exists():
        return None
    
    with open(pkg_file, 'r') as f:
        data = json.load(f)
    
    scripts = data.get("scripts", {})
    if "dev" in scripts:
        return ["npm", "run", "dev"]
    elif "start" in scripts:
        return ["npm", "start"]
    return None

def start_server(port=3000):
    if PID_FILE.exists():
        try:
            pid = int(PID_FILE.read_text().strip())
            if is_running(pid):
                print(f"⚠️  Preview já está em execução (PID: {pid})")
                return
        except:
            pass  # Arquivo PID inválido

    root = get_project_root()
    cmd = get_start_command(root)
    
    if not cmd:
        print("❌ Nenhum script 'dev' ou 'start' encontrado no package.json")
        sys.exit(1)
    
    # Adiciona a variável de ambiente de porta se necessário (heurística simples)
    env = os.environ.copy()
    env["PORT"] = str(port)
    
    print(f"🚀 Iniciando preview na porta {port}...")
    
    with open(LOG_FILE, "w") as log:
        process = subprocess.Popen(
            cmd,
            cwd=str(root),
            stdout=log,
            stderr=log,
            env=env,
            shell=True  # Necessário para npm no Windows ou para consistência no PATH
        )
    
    PID_FILE.write_text(str(process.pid))
    print(f"✅ Preview iniciado! (PID: {process.pid})")
    print(f"   Logs: {LOG_FILE}")
    print(f"   URL: http://localhost:{port}")

def stop_server():
    if not PID_FILE.exists():
        print("ℹ️  Nenhum servidor de preview encontrado.")
        return

    try:
        pid = int(PID_FILE.read_text().strip())
        if is_running(pid):
            # Tenta finalizar de forma graciosa primeiro
            os.kill(pid, signal.SIGTERM) if sys.platform != 'win32' else subprocess.call(
                ['taskkill', '/F', '/T', '/PID', str(pid)]
            )
            print(f"🛑 Preview interrompido (PID: {pid})")
        else:
            print("ℹ️  O processo não estava em execução.")
    except Exception as e:
        print(f"❌ Erro ao parar o servidor: {e}")
    finally:
        if PID_FILE.exists():
            PID_FILE.unlink()

def status_server():
    running = False
    pid = None
    url = "Desconhecida"
    
    if PID_FILE.exists():
        try:
            pid = int(PID_FILE.read_text().strip())
            if is_running(pid):
                running = True
                # Heurística para URL; idealmente deveríamos salvá-la
                url = "http://localhost:3000"
        except:
            pass
            
    print("\n=== Status do Preview ===")
    if running:
        print(f"✅ Status: Em execução")
        print(f"🔢 PID: {pid}")
        print(f"🌐 URL: {url} (Provável)")
        print(f"📝 Logs: {LOG_FILE}")
    else:
        print("⚪ Status: Parado")
    print("========================\n")

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("action", choices=["start", "stop", "status"])
    parser.add_argument("port", nargs="?", default="3000")
    
    args = parser.parse_args()
    
    if args.action == "start":
        start_server(int(args.port))
    elif args.action == "stop":
        stop_server()
    elif args.action == "status":
        status_server()

if __name__ == "__main__":
    main()
```
