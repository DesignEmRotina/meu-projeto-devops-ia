--- 
name: avaliação-de-testes-de-desempenho-de-IA
description: "Você é um especialista em revisão de código com IA, combinando análise estática automatizada, reconhecimento inteligente de padrões e práticas modernas de DevOps. Utilize ferramentas de IA (GitHub Copilot, Qodo, GPT-5, C)"
risk: desconhecido
source: comunidade
date_add: "27/02/2026"
---
...
review_prompt = f"""
Você está revisando Uma solicitação de pull request para um aplicativo {language} {project_type}.
...
Foco em:
1. Vulnerabilidades de segurança não detectadas por ferramentas de análise estática
...
5. Testabilidade e cobertura de testes ausente
...
VerificarIdempotência booleana // Tratamento de eventos duplicados?
...
5. Lacunas na proteção contra ataques de preenchimento de credenciais
...
`,
referências: ["https://cwe.mitre.org/data/definitions/89.html"],
...
run: |
CRÍTICO=$(jq '[.[] | select(.severity == "CRÍTICO")] | length' review-comments.json)

if [ $CRÍTICO -gt 0 ]; then

echo "❌ Encontrados $CRÍTICOS problemas críticos"
...
## Exemplo completo: Automação de revisão com IA
...
retornar [ReviewIssue(**issue) para issue em json.loads(content.strip())]

def post_review_comments(self, issues: List[ReviewIssue]):

resumo = "## 🤖 Revisão de Código de IA\n\n"

por_gravidade = {}

para issue em issues:

por_gravidade.setdefault(issue.severity, []).append(issue)

para gravidade em ['CRITICAL', ['ALTA', 'MÉDIA', 'BAIXA']:

contagem = len(por_gravidade.get(gravidade, []))

se contagem > 0:

resumo += f"- **{gravidade}**: {contagem}\n"

contagem_crítica = len(por_gravidade.get('CRÍTICA', []))

dados_avaliação = {

'corpo': resumo,

'evento': 'SOLICITAÇÃO_DE_ALTERAÇÕES' se contagem_crítica > 0 senão 'COMENTÁRIO',

'comentários': [issue.to_github_comment() para issue em issues]

}

# Publicar na API do GitHub

print(f"✅ Avaliação publicada com {len(issues)} comentários")
...
2. Modelos de linguagem de última geração (GPT-5, Claude 4.5 Sonnet)
3. Integração perfeita com CI/CD (GitHub Actions, GitLab, Azure DevOps)
4. Suporte para mais de 30 linguagens com linters específicos para cada uma
5. Comentários de revisão acionáveis ​​com exemplos de gravidade e correção
6. Rastreamento de métricas DORA para avaliar a eficácia da revisão
7. Gates de qualidade que previnem código de baixa qualidade
8. Geração automática de testes via Qodo/CodiumAI

Use esta ferramenta para transformar a revisão de código de um processo manual para uma garantia de qualidade automatizada com IA, detectando problemas precocemente e fornecendo feedback instantâneo.
