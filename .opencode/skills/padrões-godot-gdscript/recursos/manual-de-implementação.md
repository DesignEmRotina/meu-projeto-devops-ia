# Guia de Implementação de Padrões GDScript para Godot

Este arquivo contém padrões detalhados, listas de verificação e exemplos de código referenciados pela skill.

# Padrões GDScript para Godot

Padrões de produção para desenvolvimento de jogos com GDScript no Godot 4.x, abrangendo arquitetura, sinais, cenas e otimização.

## Quando usar esta habilidade

- Construindo jogos com Godot 4
- Implementando sistemas de jogo em GDScript
- Projetando a arquitetura da cena
- Gerenciando o estado do jogo
- Otimizando o desempenho do GDScript
- Aprendendo as melhores práticas do Godot

## Conceitos básicos

### 1. Arquitetura do Godot

```
Nó: Bloco de construção básico
├── Cena: Árvore de nós reutilizável (salva como .tscn)
├── Recurso: Contêiner de dados (salvo como .tres)
├── Sinal: Comunicação de eventos
└── Grupo: Categorização de nós
```

### 2. Noções básicas de GDScript
...
# Exportações (editáveis ​​pelo Inspector)
...
# Onready (inicializado quando pronto)
@onready var sprite: Sprite2D = $Sprite2D
@onready var animation: AnimationPlayer = $AnimationPlayer
@onready var hitbox: Area2D = $Hitbox

# Variáveis ​​privadas (convenção: prefixo com sublinhado)
var _health: int
...
velocidade = direção * velocidade

mover_e_deslizar()

func sofrer_dano(quantidade: int) -> void:

var dano_real := int(quantidade * (1.0 - redução_de_dano))

_saúde = max(_saúde - dano_real, 0)

saúde_alterada.emit(_saúde)
...
```gdscript
# 1. Armazene em cache as referências dos nós
@onready var sprite := $Sprite2D # Bom
# $Sprite2D em _process() # Ruim - busca repetida

# 2. Use o pool de objetos para geração frequente
# Veja o Padrão 4

# 3. Evite alocações em caminhos críticos
var _reusable_array: Array = []

func _process(_delta: float) -> void:

_reusable_array.clear() # Reutilize em vez de criar um novo

# 4. Use tipagem estática
func calculate(value: float) -> float: # Bom

retorne o valor * 2.0

# 5. Desabilite o processamento quando não for necessário
...
### Recomendações
- **Use sinais Para desacoplamento** - Evite referências diretas
- **Tipagem estática** - A tipagem estática detecta erros
- **Use recursos para dados** - Separe dados da lógica
- **Agrupe objetos criados com frequência** - Evite gargalos de coleta de lixo
- **Use Autoloads com moderação** - Apenas para sistemas verdadeiramente globais

### O que não fazer
- **Não use `get_node()` em loops** - Armazene referências em cache
- **Não acople cenas fortemente** - Use sinais
- **Não coloque lógica em recursos** - Mantenha-os apenas para dados
- **Não ignore o Profiler** - Monitore o desempenho
- **Não lute contra a árvore de cenas** - Trabalhe com o design do Godot

## Recursos

- [Documentação do Godot](https://docs.godotengine.org/en/stable/)
- [Tutoriais do GDQuest](https://www.gdquest.com/)
- [Godot Receitas](https://kidscancode.org/godot_recipes/)