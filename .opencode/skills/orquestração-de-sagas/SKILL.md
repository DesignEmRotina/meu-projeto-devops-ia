--- 
name: orquestração-de-sagas
description: "Padrões para gerenciar transações distribuídas e processos de negócios de longa duração."
risk: desconhecido
source: comunidade
date_add: "27/02/2026"
---
...
Padrões para gerenciar transações distribuídas e processos de negócios de longa duração.

## Não use esta habilidade quando

- A tarefa não estiver relacionada à orquestração de sagas
...
- Se forem necessários exemplos detalhados, abra `recursos/manual-de-implementação.md`.

## Use esta habilidade quando:

- Coordenar transações multisserviços
- Implementar transações compensatórias
- Gerenciar fluxos de trabalho de longa duração
- Lidar com falhas em sistemas distribuídos
- Construir processos de atendimento de pedidos
- Implementar fluxos de trabalho de aprovação
...
┌─────┐ ┌─────┐ ┌─────┐ ┌─────────────┐
│Serviço A│─►│Serviço B│─►│Serviço C│ │ Orquestrador│
└─────┘ └─────┘ └─────┘ └──────┬──────┘
...
### 2. Estados de Execução da Saga

| Estado | Descrição |

| ---------------- | ------------------------------ |

| **Iniciada** | Saga iniciada |

| **Pendente** | Aguardando a conclusão da etapa |

| **Compensando** | Revertendo devido a falha |

| **Concluída** | Todas as etapas foram concluídas com sucesso |

| **Falha** | Saga fracassou após indenização |
...
## Alternativa de Execução Durável

Os modelos acima constroem a infraestrutura de sagas do zero — repositórios de sagas, publicadores de eventos, rastreamento de compensação. **Frameworks de execução durável** (como o DBOS) eliminam grande parte desse código repetitivo: o ambiente de execução do fluxo de trabalho persiste automaticamente o estado em um banco de dados, repete etapas com falha e retoma do último ponto de verificação após falhas. Em vez de criar uma classe base `SagaOrchestrator`, você escreve uma função de fluxo de trabalho com etapas — o framework lida com a persistência, a recuperação de falhas e a semântica de execução exatamente uma vez. Considere a execução durável quando desejar a confiabilidade de uma saga sem precisar gerenciar a infraestrutura de coordenação por conta própria.

## Melhores Práticas

### Recomendações

- **Torne os passos idempotentes** - É seguro tentar novamente
- **Projete as compensações cuidadosamente** - Elas devem funcionar
- **Use IDs de correlação** - Para rastrear entre serviços
- **Implemente timeouts** - Não espere indefinidamente
- **Registre tudo** - Para depurar falhas

### Recomendações Não Faça

- **Não assuma conclusão instantânea** - Sagas levam tempo
- **Não ignore os testes de compensação** - Parte mais crítica
- **Não acople serviços** - Use mensagens assíncronas
- **Não ignore falhas parciais** - Lide com elas de forma elegante

## Habilidades Relacionadas

Funciona bem com: `event-sourcing-architect`, `workflow-automation`, `dbos-*`

## Recursos

- [Padrão Saga](https://microservices.io/patterns/data/saga.html)
- [Designing Data-Intensive](https://microservices.io/patterns/data/saga.html) Aplicações](https://dataintensive.net/)