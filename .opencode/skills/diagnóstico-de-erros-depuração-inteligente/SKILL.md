--- 
nome: diagnóstico-de-erros-depuração-inteligente
descrição: "Use ao trabalhar com o recurso de depuração inteligente de diagnóstico de erros"
risco: desconhecido
origem: comunidade
data_de_adição: "2026-02-27"
---

## Use esta habilidade quando

- Estiver trabalhando em tarefas ou fluxos de trabalho de depuração inteligente de diagnóstico de erros
- Precisar de orientação, melhores práticas ou listas de verificação para a depuração inteligente de diagnóstico de erros

## Não use esta habilidade quando

- A tarefa não estiver relacionada à depuração inteligente de diagnóstico de erros
...
- Evidências de suporte de logs/rastreamentos/código
...
### 4. Estratégia Seleção
Selecione com base nas características do problema:

**Depuração Interativa**: Reproduzível localmente → VS Code/Chrome DevTools, execução passo a passo
**Orientado à Observabilidade**: Problemas em produção → Sentry/DataDog/Honeycomb, análise de rastreamento
**Viagem no Tempo**: Problemas de estado complexos → rr/Redux DevTools, gravação e reprodução
**Engenharia do Caos**: Intermitente sob carga → Chaos Monkey/Gremlin, injeção de falhas
**Estatístico**: Pequena porcentagem de casos → Depuração delta, comparação entre sucesso e falha
...
Use breakpoints e logpoints condicionais para ambientes semelhantes a produção.
...
**Distribuição Gradual de Tráfego**: Implantação canary da versão de depuração para 10% do tráfego

### 7. Análise da Causa Raiz
Análise do fluxo de código com IA:
...
- Detecção de "code smells" (problemas de código)
...
- Nível de risco
- Necessidades de cobertura de testes
...
- Implantação canary (monitoramento da taxa de erros)
- Revisão do código corrigido pela IA
...
## Exemplo: Depuração Mínima Sessão
...
frequência: "5% dos checkouts",

ambiente: "produção"
});

// Sugestão da IA: "Provavelmente tempo limite de consulta N+1 ou API externa"
